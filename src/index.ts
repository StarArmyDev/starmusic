...
export default class StarMusic extends Music {
                    ...
                    .then((response) => {
                        this._youtube
                            ...
                            .then((searchResult) => {
                                ...
                                const startTheFun = async (videos: ICancion[], max: number): Promise<void> => {
                                    if (message.channel.type != 'dm' && message.channel.permissionsFor(message.guild!.me!)!.has('EMBED_LINKS')) {
                                        ...
                                        message.channel.send(embed).then((firstMsg) =>
                                            message.channel
                                                .awaitMessages(
                                                    (m: Message) => {
                                                        const contents: boolean[] = [];
                                                        for (let i = 0; i < max; i++) contents.push(m.content.includes((i + 1).toString()));
    ...
    radio(message: Message, stream?: string): void {
        ...
        let servidores = this._guilds.get(message.guild.id);
        if (message.member && !message.member.voice.channel) message.channel.send(this.notaMsg('fail', 'No estas en un canal de voz.'));
        ...
        this.connectBot(message)
            .then(async (connection) => {
                if (stream && stream.length > 1)
                    if (this.isStreamValid(stream)) message.channel.send(this.notaMsg('fail', 'Link de radio inválido.'));
                    else
                        try {
    ...
    pause(message: Message): void {
        ...
        if (!voiceConnection || !servidores?.last) message.channel.send(this.notaMsg('fail', 'No se está reproduciendo música.'));
        else if (message.guild! && servidores.isRadio) message.channel.send(this.notaMsg('fail', 'No se puede usar en modo radio.'));
        ...
        else if (voiceConnection.dispatcher?.paused) message.channel.send(this.notaMsg('fail', '¡La música ya está en pausa!'));
        else {
    ...
    resume(message: Message): void {
        ...
        else if (!voiceConnection.dispatcher?.paused) message.channel.send(this.notaMsg('fail', 'La música no está páusada.'));
        else {
    ...
    skip(message: Message): void {
        ...
        else if (servidores.loop == 'single') message.channel.send(this.notaMsg('fail', 'No se puede omitir mientras que el bucle está configurado como `Una canción`'));
        else if (!voiceConnection.dispatcher) message.channel.send(this.notaMsg('fail', 'No se está reproduciendo música.'));
        else {
    ...
    np(message: Message): void {
        ...
        const voiceConnection = message.client.voice?.connections.find((val) => val.channel.guild.id == message.guild?.id);
        const servidores = this._guilds.get(message.guild.id);
        if (!voiceConnection) message.channel.send(this.notaMsg('fail', 'No hay Música sonando.'));
        else if (!servidores?.last) message.channel.send(this.notaMsg('note', 'Cola vacía.'));
        else if (servidores.isRadio) message.channel.send(this.notaMsg('fail', 'No se puede usar en modo radio.'));
        else {
            ...
            if (message.channel.type != 'dm' && message.channel.permissionsFor(message.guild!.me!)!.has('EMBED_LINKS')) {
                const embed = new MessageEmbed()
                    ...
                    .addField(
                        '🔊Escuchando:',
                        `**${servidores.last.title}**\n[YouTube](${servidores.last.url}) por: 👤[Canal](https://www.youtube.com/channel/${servidores.last.channelId})`,
                        true
                    )
                    .addField('⏭En Cola', servidores.songs.length);
                if (this._show_name && resMem) embed.setFooter(`Solicitado por: ${resMem.username}`, resMem.displayAvatarURL());
                if (this._show_name && !resMem) embed.setFooter(`Solicitado por: \`Usuario desconocido (ID: ${servidores.last.autorID})\``);
    ...
    async repeat(message: Message, song?: 0 | 1 | 2 | 3): Promise<void> {
        ...
        if (!servidores) message.channel.send(this.notaMsg('fail', 'No se ha encontrado ninguna cola para este servidor'));
            ...
            if (servidores.isRadio) message.channel.send(this.notaMsg('fail', 'No se puede usar en modo radio.'));
                ...
                if (servidores.last) {
                    ...
                    if (newq !== servidores.songs) servidores = await this.updatePositions(message.guild, servidores, newq);
                }
    ...
    queue(message: Message, song?: number): void {
        if (!message.guild || !message.member) return undefined;
        ...
        if (!servidores) message.channel.send(this.notaMsg('fail', 'No se pudo encontrar una cola para este servidor.'));
            ...
            if (song) {
                ...
                if (!video) message.channel.send(this.notaMsg('fail', 'No pude encontrar ese video.'));
                else {
                    embed
                    ...
                    if (this._show_name && resMem) embed.setFooter(`Solicitado por: ${resMem.username}`, resMem.displayAvatarURL());
                    if (this._show_name && !resMem) embed.setFooter(`Solicitado por: \`Usuario desconocido (ID: ${video.autorID})\``);
                    message.channel.send({ embed });
                }
                ...
                message.channel.send(embed).then(async (m) => {
                    ...
                    m.createReactionCollector((reaction: MessageReaction, user: User) => reaction.emoji.name === '⏩' && user.id === message.author.id, {
                        time: 120000
                    }).on('collect', () => {
    ...
    remove(message: Message, song: number): void {
        ...
        else if (song - 1 == 0) message.channel.send(this.notaMsg('fail', 'No puedes borrar la música que se está reproduciendo actualmente.'));
        else {
            const cancionR = servidores.songs.find((x) => x.position == song - 1);

            if (!cancionR) message.channel.send(this.notaMsg('fail', 'No se pudo encontrar ese video o algo salió mal.'));
            else if (cancionR.autorID == message.author.id || this.isAdmin(message.member))
                this.updatePositions(
                    message.guild!,
                    servidores,
                    servidores.songs.filter((s) => s !== cancionR)
                ).then((res) => {
                    this._guilds.set(message.guild!.id, res);
                    message.channel.send(this.notaMsg('note', `Eliminado:  \`${cancionR.title}\``));
                });
            else message.channel.send(this.notaMsg('fail', 'No puedes eliminar esta canción.'));
        }
    }

    volume(message: Message, volume: number): void {
        if (!message.guild || !message.member) return undefined;
    ...
    clear(message: Message): void {
        ...
        const servidores = this._guilds.get(message.guild.id);
        if (!servidores || !servidores.last) message.channel.send(this.notaMsg('fail', 'No se ha encontrado ninguna cola para este servidor..'));
        else if (this._just_dj && !this.isDj(message.member) && !this.isAdmin(message.member))
