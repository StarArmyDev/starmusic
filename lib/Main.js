...
class StarMusic extends Music_1.default {
    async play(message, search) {
        ...
        if (!message.guild || !member)
            this.sendReply(message, this.notaMsg('fail', 'No estas en un servidor.'));
        else if (!member.voice?.channel)
            this.sendReply(message, this.notaMsg('fail', 'No estas en un canal de voz.'));
        else if (!search)
            this.sendReply(message, this.notaMsg('fail', '¡No has colocado nada que buscar!'));
            ...
            if (subscription?.queue.length >= this._max_tail && this._max_tail > 0)
                this.sendReply(message, this.notaMsg('fail', 'Tamaño máximo de cola alcanzado'));
            else if (subscription?.queueLock)
                this.sendReply(message, this.notaMsg('fail', 'Procesando una solicitud previa, intente de nuevo en unos segundos.'));
            else {
                ...
                if (searchstring.startsWith('http') && searchstring.includes('list=')) {
                    ...
                    ytpl_1.default(playid)
                        ...
                        try {
                            ...
                            if (ran >= result.items.length)
                                if (index == 0)
                                    this.sendReply(message, this.notaMsg('fail', 'No pude obtener ninguna canción de esa lista de reproducción'));
                                else if (index == 1)
                        ...
                        .catch(() => this.sendReply(message, this.notaMsg('fail', 'Algo salió mal al buscar esa lista de reproducción')));
                }
                else {
                    ...
                    this.createSong(message, searchstring)
                        ...
                        .catch(() => this.sendReply(message, this.notaMsg('fail', 'No se econtró ningún video.')));
                }
            }
        }
    }
                ...
                message
                    ...
                    this._youtube
                        ...
                        if (searchResult.results.length == 0) {
                            ...
                            if (response)
                                response.edit(mensaje);
                            else
                                message.editReply(mensaje);
                            return;
                        }
                        const promesas = [];
                        const videos = [];
                        ...
                        try {
                            ...
                            const process = (firstMsg) => {
                                message.channel
                                    ...
                                    if (['cancel', 'cancelar'].includes(mcon)) {
                                        const mensaje = { content: '````xl\nBúsqueda cancelada.\n```', embeds: [] };
                                        if (response)
                                            firstMsg.edit(mensaje);
                                        else
                                            message.editReply(mensaje);
                                    }
                                    firstMsg.delete();
                            ...
                            if (message.channel.type == 'dm' || message.channel.permissionsFor(message.guild.me).has('EMBED_LINKS')) {
                                ...
                                if (this._show_name)
                                    embed.setFooter(`Buscado por: ${message.member.user.username}`, message.author
                                        ? message.author.displayAvatarURL()
                                        : message.user.displayAvatarURL());
                                if (response)
                                    message.channel.send({ embeds: [embed] }).then((m) => process(m));
                                else
                                    message.followUp({ embeds: [embed] }).then(async (m) => process(m));
                            }
                            else {
                                const vids = videos
                                    .map((video_4, index_1) => `**${index_1 + 1}:** __${video_4.title
                                    .replace(/\\/g, '\\\\')
                                    .replace(/`/g, '\\`')
                                    .replace(/\* /g, '\\*')
                                    .replace(/_/g, '\\_')
                                ...
                                if (response)
                                    message.channel.send(mensaje).then((m) => process(m));
                                else
                                    message.followUp(mensaje).then((m) => process(m));
                            }
                        }
                        catch (err) {
    ...
    async pause(message) {
        if (message.commandID)
            await message.defer({ ephemeral: true });
        const member = message.member;
        if (!message.guild || !member?.voice)
            return;
        const song = this.getSong(message);
        const subscription = this._subscriptions.get(message.guild.id);
        if (!song)
            this.sendReply(message, this.notaMsg('fail', 'No se está reproduciendo música.'));
            ...
            this.sendReply(message, this.notaMsg('fail', '¡La música ya está en pausa!'));
        else {
    ...
    async resume(message) {
        ...
        if (!subscription)
            this.sendReply(message, this.notaMsg('fail', 'No se está reproduciendo música'));
        else if (!this._any_pause && song.autorID != member.id && !this.isAdmin(member))
            this.sendReply(message, this.notaMsg('fail', 'No tienes permiso de reanudar.'));
        else if (subscription.audioPlayer.state.status != voice_1.AudioPlayerStatus.Paused)
            this.sendReply(message, this.notaMsg('fail', 'La música no está páusada.'));
        else {
    ...
    async skip(message) {
        ...
        if (!song || !subscription)
            ...
            this.sendReply(message, this.notaMsg('fail', 'No puedes omitir la canción porque no hay una cola de reproducción.'));
    ...
    async repeat(message, song) {
        ...
        if (song && (song < 0 || song > 3))
            this.sendReply(message, this.notaMsg('fail', 'Solamente puedes colocar 0, 1, 2 o 3'));
        const subscription = this._subscriptions.get(message.guild.id);
        if (!subscription)
            this.sendReply(message, this.notaMsg('fail', 'No se ha encontrado ninguna cola para este servidor'));
    ...
    async queue(message, songSearch) {
        ...
        if (!subscription)
            ...
            if (songSearch) {
                ...
                if (message.commandID)
                    message.reply({ embeds: [embed], ephemeral: true }).then(async () => response((await message.fetchReply())));
                else
                    message.channel.send({ embeds: [embed] }).then((m) => response(m));
                const response = async (m) => {
                    await m.react('⏪');
                    await m.react('⏩');
                    m.createReactionCollector({ filter: (reaction, user) => reaction.emoji.name === '⏩' && user.id === message.member.user.id, time: 120000 }).on('collect', () => {
                        if (page === pages.length)
                            return;
                        page++;
                        embed.setDescription(pages[page - 1]);
                        embed.setFooter(`Página ${page} de ${pages.length}`, message.author ? message.author.displayAvatarURL() : message.user.displayAvatarURL());
                        m.edit({ embeds: [embed] });
                    });
                    m.createReactionCollector({ filter: (reaction, user) => reaction.emoji.name === '⏪' && user.id === message.member.user.id, time: 120000 }).on('collect', () => {
                        if (page === 1)
                ...
                embed
                    ...
                    .setFooter('Página 1 de 1', message.author ? message.author.displayAvatarURL() : message.user.displayAvatarURL());
                message.reply({ embeds: [embed] });
            }
        }
    }
