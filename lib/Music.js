...
class Music {
    constructor(options) {
        ...
        this._embed_color = libs_1.resolveColor(options.embedColor || 'GREEN');
        ...
        this._laut = new lautfm_1.default();
    ...
    sendMessage(message, video) {
        ...
        if (message.channel.type != 'dm' && message.channel.permissionsFor(message.guild.me).has('EMBED_LINKS')) {
            ...
            if (this._show_name && !resMem)
                solicitado = `Solicitado por: \`Usuario desconocido (ID: ${video.autorID})\``;
